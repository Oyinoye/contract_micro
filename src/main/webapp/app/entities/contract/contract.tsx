  ...
  return (
    <div>
      <h2 id="contract-heading" data-cy="ContractHeading">
        <Translate contentKey="maxContractApp.contract.home.title">Contracts</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="maxContractApp.contract.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="maxContractApp.contract.home.createLabel">Create new Contract</Translate>
          </Link>
        </div>
      </h2>
        ...
        {contractList && contractList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="maxContractApp.contract.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {contractList.map((contract, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{contract.championID}</td>
                  <td>{contract.vehicleID}</td>
                  <td>{contract.hpAmount}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="maxContractApp.contract.home.notFound">No Contracts found</Translate>
            </div>
          )
        )}
      </div>
